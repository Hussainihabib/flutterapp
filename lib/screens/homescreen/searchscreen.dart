      ...
      body: FutureBuilder(
        ...
        builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
          if (!snapshot.hasData) {
            ...
            // NEW: smart filter for name, brand, and price
            var filtered = data.where((element) {
              String name = element['p_name'].toString().toLowerCase();
              String brand = element['p_category'].toString().toLowerCase();
              String sub = element['p_subcategories'].toString().toLowerCase();

              String price = element['p_price'].toString();

              String search = title!.toLowerCase();
              bool isNumeric = double.tryParse(title!) != null;

              if (isNumeric) {
                // agar number hai, price match kare
                return price.contains(title!);
              } else {
                // otherwise name ya brand match kare
                return name.contains(search) || brand.contains(search)  || sub.contains(search) ; 
              }
            }).toList();

            print(data[0]['p_name']);
            print(data[0]['p_price']);
            print(data[0]['p_category']);
            print(data[0]['p_category']);
